...
```{r,eval=F}
opd_teacher <- select(opd_survey, Role, Resource...6, Q21)
...
```{r}
opd_tidy <- unnest_tokens(opd_teacher, word, text) #for the data, create 'word' column, from text
...
opd_tf_idf %>%
  ...
  ungroup() %>%
  mutate(Resource...6=as.factor(Resource...6),
